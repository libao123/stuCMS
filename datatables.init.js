	...
	buildSearch: function ($ctrl) {
		...
		if ($ctrl['cols'] != undefined && $ctrl['cols']) {
			...
			var buildEach = function (d) {
				...
				if (d['type'] == "select") {
					s += '<select class="form-control" data-column="' + d['col'] + '" name="' + d['data'] + '" id="search-' + d['data']
                    + '" ddl_name="' + d['ddl_name'] +'" show_type="t" pre="' + d['pre']
                    + '" d_value="' + d['d_value'] + '" style="margin-right:6px;">';
					if (d['options']) {
			...
			str += '<div class="form-group">' +
				'<button type="button" class="btn btn-primary btn-flat btn-search" style="margin-right:6px;">搜索</button></div>' +
				'<div class="form-group"><button type="button" class="btn btn-default btn-flat btn-clearData">清除</button></div>' +
				'</div>';
