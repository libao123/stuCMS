...
var JzCom=
{
    ...
    ByJzTypeToPrint:function (type,apply_oid,str_file_oid)
    {
        ...
        if (type == "COUNTRY_B"){
            window.open(OptimizeUtils.FormatUrl(url+ 'Print_11.aspx?optype=print&id=' + apply_oid));
            window.open(OptimizeUtils.FormatUrl(url+ 'Print_11_2.aspx?optype=print&id=' + apply_oid));
            //附件
            for(var i = 0; i < arr_file_oid.length; i++){
                if (arr_file_oid[i] == null || arr_file_oid[i].length == 0)
                    continue;
                window.open(OptimizeUtils.FormatUrl(url+'PrintFile.aspx?optype=print&id=' + arr_file_oid[i]));
            }
        }
        //COUNTRY_ENCOUR：国家励志奖学金：表12
        //AREA_GOV：自治区人民政府奖学金：表15
        else if (type == "COUNTRY_ENCOUR" || type == "AREA_GOV"){
            window.open(OptimizeUtils.FormatUrl(url+ 'Print_12_15.aspx?optype=print&id=' + apply_oid));
        }
        ...
        //SOCIETY_OFFER：社会捐资类奖学金：表16
            ...
            window.open(OptimizeUtils.FormatUrl(url+ 'Print_13_14_16.aspx?optype=print&id=' + apply_oid));
        }
        //SCHOOL_GOOD：三好学生：表17+1
        //SCHOOL_MODEL：三好学生标兵：表17+1,17+2（与11+2一致）,17+3
        else if (type == "SCHOOL_GOOD" || type == "SCHOOL_MODEL")
        {
            window.open(OptimizeUtils.FormatUrl(url+'Print_17.aspx?optype=print&id=' + apply_oid));
        ...
        //SCHOOL_SINGLE：单项奖学金：表18
            ...
            window.open(OptimizeUtils.FormatUrl(url+ 'Print_18.aspx?optype=print&id=' + apply_oid));
        }
        ...
        //SOCIETY_NOCOUNTRY：非国家级奖学金：表21
            ...
            window.open(OptimizeUtils.FormatUrl(url+ 'Print_19_20_21.aspx?optype=print&id=' + apply_oid));
            if (type == "COUNTRY_YP") {
                window.open(OptimizeUtils.FormatUrl(url+'Print_19_2.aspx?optype=print&id=' + apply_oid));
            }
            //附件
            for(var i = 0; i < arr_file_oid.length; i++){
                if (arr_file_oid[i] == null || arr_file_oid[i].length == 0)
