	...
	buildSearch: function ($ctrl) {
		...
		if ($ctrl['cols'] != undefined && $ctrl['cols']) {
			...
			var buildEach = function (d) {
				...
				if (d['type'] == "select") {
					s += '<select class="form-control" data-column="' + d['col'] + '" name="' + d['data'] + '" id="search-' + d['data'] + '" ddl_name="' + d['ddl_name'] +'" d_value="" show_type="t" pre="' + d['pre'] + '" style="margin-right:6px;">';
					if (d['options']) {
...
//				var str = '<aside class="control-sidebar control-sidebar-dark" id="">\
//			    <ul class="nav nav-tabs nav-justified control-sidebar-tabs">\
//			      <li class="">\
//			        <a href="#'+searchId+'" data-toggle="tab"><i class="fa fa-search"></i></a>\
//			      </li>\
//			    </ul>\
...
//			};
			...
			var buildBox = function(){
				var s = '<div class="box box-default collapsed-box" id="'+boxId+'">\
			...
			prop.searchs = str;
			// $("body").append(buildBox());
			var a  = $("#" + elo.property.tableId).parent();
			a.before(buildBox())
			/* new */
			// var tabLi = '<li class="">\
      //   <a href="#'+tabId+'" data-toggle="tab"><i class="fa fa-search">'+tabMsg+'</i></a>\
      // </li>';
			// var tabContent = '<div class="tab-pane" id="'+tabId+'">\
			// 	<h3 class="control-sidebar-heading">查询：</h3>\
			// </div>';
			// $search.find(".control-sidebar-tabs").append(tabLi);
			// $search.find(".tab-content").append(tabContent);
			// $search.find("#"+tabId).append(str);
			// $("[data-toggle='control-sidebar']").on("click", function(){
			// 	try {
			// 		console.log($(this).attr('data-id'));
			// 		var tid = $(this).attr('data-id');
			// 		// console.log($search.find(".control-sidebar-tabs").find('li'));
			// 		$search.find(".control-sidebar-tabs").find('li').each(function(){
			// 			if($(this).find('a').attr("href") == "#"+tid){
			// 				$(this).addClass("active");
			// 			}
			// 		});
			// 		$search.find(".tab-content").find('.tab-pane').each(function(){
			// 			if($(this).attr("id") == tid){
			// 				$(this).addClass("active");
			// 			}
			// 		});
...
//			$("#" + prop.tableId).parent().on('click', searchBtn, function () {
//				//按钮搜索方法
//				search(this, elo);
//			});
//			$("#"+prop.tableId).parent().on("click", ".btn-clearData", function(){
//				var _ts = $(this);
//				var $searchBox = $(".search-box");
//				//console.log($(this));
//				$searchBox.find("input, select, textarea").val("");
	...
	createOne:	function(sData){
		...
		$table.init = function(){
			...
			dataConfig['initComplete'] = function(){
					...
					$("#" + property.tableId).parent().find(".search-box select").each(function () {
						if ($(this).attr("ddl_name") && $(this).attr("ddl_name") != "undefined")
                        {
							DropDownUtils.initDropDown($(this).attr("id"),$(this).attr("pre"));
                        }
