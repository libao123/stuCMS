<%@ Page Title="" Language="C#" MasterPageFile="~/Master/AltMaster.Master" AutoEventWireup="true" CodeBehind="JobManage.aspx.cs" Inherits="PorteffAnaly.Web.AdminLTE_QZ.BasicSetup.JobManage" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="Main" runat="server">
        ...
        <div class="modal-dialog modal-dw60">
            <form action="#" method="post" id="form_edit" name="form_edit" class="modal-content form-horizontal"
                ...
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">岗位申报基本设置</h4>
                </div>
                <div class="modal-body">
                    ...
                    <div class="form-group col-sm-12">
                        <label class="col-sm-2 control-label">
                            申报时间<span style="color: Red;">*</span></label>
                    ...
                    <div class="form-group col-sm-12">
                        <label class="col-sm-2 control-label">
                            本科生报酬标准<span style="color: Red;">*</span></label>
                        <div class="col-sm-4" style="position: relative;">
                            <input type="text" name="REWARD_STD_B" id="REWARD_STD_B" class="form-control" />
                        </div>
                        <label class="col-sm-2 control-label">
                            计薪单位<span style="color: Red;">*</span></label>
                        <div class="col-sm-4" style="position: relative;">
                            <select class="form-control" name="REWARD_UNIT_B" id="REWARD_UNIT_B" d_value='' show_type='t'
                    ...
                    <div class="form-group col-sm-12">
                        <label class="col-sm-2 control-label">
                            研究生报酬标准<span style="color: Red;">*</span></label>
                        <div class="col-sm-4" style="position: relative;">
                            <input type="text" name="REWARD_STD_Y" id="REWARD_STD_Y" class="form-control" />
                        </div>
                        <label class="col-sm-2 control-label">
                            计薪单位<span style="color: Red;">*</span></label>
                        <div class="col-sm-4" style="position: relative;">
                            <select class="form-control" name="REWARD_UNIT_Y" id="REWARD_UNIT_Y" d_value='' show_type='t'
                                ddl_name='ddl_reward_unit'>
                            </select>
                        </div>
                    </div>
    ...
    <script type="text/javascript">
        ...
        function loadTableList() {
            ...
            tablePackageMany.filed = [
				...
				{ "data": "DECLARE_TIME", "head": "申报时间范围", "type": "td-keep" },
    ...
    <script type="text/javascript">
        ...
        function loadModalPageDataInit() {
            ...
            DropDownUtils.initDropDown("REWARD_UNIT_B");
            DropDownUtils.initDropDown("REWARD_UNIT_Y");
            LimitUtils.onlyNum("REWARD_STD_B");
            LimitUtils.onlyNum("REWARD_STD_Y");
            //用户角色
            GetUserRoleHtml();
            //设置checkbox选中改变事件
        ...
        function loadModalPageValidate() {
            ...
            ValidateUtils.setRequired("#form_edit", "DECLARE_START_TIME", true, "开始申报时间必填");
            ValidateUtils.setRequired("#form_edit", "DECLARE_END_TIME", true, "结束申报时间必填");
            ValidateUtils.setRequired("#form_edit", "REWARD_STD_B", true, "本科生报酬标准必填");
            ValidateUtils.setRequired("#form_edit", "REWARD_UNIT_B", true, "本科生报酬计算单位必填");
            ValidateUtils.setRequired("#form_edit", "REWARD_STD_Y", true, "研究生报酬标准必填");
            ValidateUtils.setRequired("#form_edit", "REWARD_UNIT_Y", true, "研究生报酬计算单位必填");
        }
    </script>
    <script type="text/javascript">
        //编辑页按钮事件初始化
                ...
                if (data) {
                    if (data.OID) {
                        if (data.NOTICE_ID) {
                            ...
                            _form_notice.reset();
                            $("#hidNOTICE_OID").val("");
                            $("#TITLE").val("勤工助学岗位申报开始进行");
                            $("#SUB_TITLE").val("申报时间 从 " + data.DECLARE_START_TIME + " -- " + data.DECLARE_END_TIME + "结束");
        ...
        //保存事件
        function SaveData() {
            var result = AjaxUtils.getResponseText("?optype=check&oid=" + $("#OID").val() + "&year=" + $("#SCH_YEAR").val() + "&term=" + $("#SCH_TERM").val());
            if (result.length > 0) {
                easyAlert.timeShow({
                    "content": result,
                    "duration": 2,
        ...
        function SendNoticeData() {
            ...
            if (data) {
                if (data.OID) {
                    ...
                    $.post(OptimizeUtils.FormatUrl("/AdminLTE_Mod/BDM/Notice/List.aspx?optype=save&job_oid=" + data.OID), $("#form_notice").serialize(), function (msg) {
                        if (msg.length != 0) {
                            $('.maskBg').hide();
                            ZENG.msgbox.hide();
                            easyAlert.timeShow({
