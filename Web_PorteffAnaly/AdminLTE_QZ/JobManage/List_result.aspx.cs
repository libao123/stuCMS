...
{
    public partial class List_result : ListBaseLoad<Qz_job_manage>
    {
        #region 初始化
        ...
        {
            string where = string.Format(" AND EXPECT_JOB1 = '{0}'", Get("job"));
            var ddl = new Hashtable();
            ddl.Add("GRADE", "ddl_grade");
            ddl.Add("COLLEGE", "ddl_department");
            ddl.Add("MAJOR", "ddl_zy");

            return tables.GetCmdQueryData("GET_JOB_APPLY_LIST", null, where, string.Empty, ddl);
        }
