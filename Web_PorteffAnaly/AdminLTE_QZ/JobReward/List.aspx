<%@ Page Title="" Language="C#" MasterPageFile="~/Master/AltMaster.Master" AutoEventWireup="true" CodeBehind="List.aspx.cs" 
    Inherits="PorteffAnaly.Web.AdminLTE_QZ.JobReward.List" %>
        ...
        <div class="modal-dialog modal-dw80">
            <form action="#" method="post" id="form_edit" name="form_edit" class="modal-content form-horizontal form-inline"
                ...
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">用人单位签领单申请</h4>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-4">
                            <p class="form-control-static" id="REWARD_STD">
                                -</p>
                            <%--<input type="text" name="REWARD_STD" id="REWARD_STD" class="form-control" placeholder="薪资标准"
                                required />--%>
                        </div>
                        <label class="col-sm-2 control-label">
                            数量</label>
                        <div class="col-sm-4">
                            <p class="form-control-static" id="QUANTITY">
                                -</p>
                            <%--<input type="text" name="QUANTITY" id="QUANTITY" class="form-control" placeholder="数量"
                                required />--%>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <label class="col-sm-2 control-label">
                            实发金额<span style="color: Red;">*</span></label>
                        <div class="col-sm-4">
                            <input type="text" name="REAL_AMOUNT" id="REAL_AMOUNT" class="form-control" placeholder="实发金额"
                                required />
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary btn-save" id="btnRewardSave">
                        保存</button>
        ...
        function loadRewardList() {
            ...
            content.on('click', ".btn-edit", function () {
                ...
                if (data) {
                    ...
                    SetMainFormData("rewardModal", "", data);
                }
                else {
                    easyAlert.timeShow({
                        "content": "请选择一条数据！",
    ...
    <script type="text/javascript">
        ...
        function loadModalBtnInit() {
            ...
            _content.on('click', '#btn-view', function () {
                ...
                if (data) {
                    $("#tableModal").modal();
                    var apply_data = AjaxUtils.getResponseText('List.aspx?optype=getdata&id=' + data.OID + '&seq_no=' + data.SEQ_NO);
                    if (apply_data) {
            ...
            $.post(OptimizeUtils.FormatUrl("?optype=save"), $("#form_edit").serialize(), function (msg) {
                if (msg.length == 0) {
                    ...
                    if (arr.length == 2) {
                        $("#OID").val(arr[0]);
                        $("#SEQ_NO").val(arr[1]);
                        rewardList.refresh("?optype=getrewardlist&seq_no=" + arr[1].toString());
                    }
                    //保存成功：刷新列表
                    easyAlert.timeShow({
                        "content": "保存成功！",
        ...
        function SaveReward() {
            if (!($("#reward_edit").valid())) {
                return;
            }
            var param = $("#reward_edit").serialize();
            $.post(OptimizeUtils.FormatUrl("?optype=savereward&seq_no=" + $("#SEQ_NO").val()), param, function (msg) {
                if (msg.length == 0) {
                    easyAlert.timeShow({
                        "content": "保存失败！",
                        "duration": 2,
