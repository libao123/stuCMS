...
{
    public partial class List_proc : ListBaseLoad<Qz_job_reward_head>
    {
        #region 初始化
                ...
                if (!string.IsNullOrEmpty(optype))
                {
                    switch (optype.ToLower().Trim())
                    {
                        ...
                        case "getrewardlist":
                            ...
                            break;
                        case "ishave":
                            Response.Write(IsHaveDownloadData());
                            Response.End();
                            break;
                    }
                }
        ...
        private string IsHaveDownloadData()
        {
            string month = Get("month");
            string employer = Get("employer");
            string job = Get("job");
            string strSql = string.Empty;
            string strWhere = string.Empty;

            if (month.Length > 0)
                strWhere += string.Format(" AND A.YEAR_MONTH = '{0}'", month);
            if (employer.Length > 0)
                strWhere += string.Format(" AND A.EMPLOYER = '{0}'", employer);
            if (job.Length > 0)
                strWhere += string.Format(" AND B.JOB_SEQ_NO = '{0}'", job);

            strSql = string.Format("SELECT * FROM QZ_JOB_REWARD_HEAD A INNER JOIN QZ_JOB_REWARD_LIST B ON A.SEQ_NO = B.SEQ_NO WHERE 1 = 1 {0}", strWhere);
