...
function Formatter_RetChannel(ret_channel, pos_name) {
    var data = {};
    if (ret_channel == 'A0000') {
        data = { type: "primary", msg: "预录入" };
    } else if (ret_channel == 'A0010') {
        data = { type: "primary", msg: "申请中" };
    } else if (ret_channel == 'D4000') {
        data = { type: "success", msg: "审批通过" };
    } else if (ret_channel.substring(0, 1) == "D" && ret_channel.substring(2) == "000") {
        data = { type: "danger", msg: pos_name + "待审" };
    } else if (ret_channel.substring(0, 1) == "D" && ret_channel.substring(2) == "010") {
        data = { type: "success", msg: pos_name + "通过" };
    } else if (ret_channel.substring(0, 1) == "D" && ret_channel.substring(2) == "020") {
        data = { type: "warning", msg: pos_name + "不通过" };
    } else {
        data = { type: "danger", msg: "未申请" };
    }
    return data;
}
