...
<asp:Content ID="Content2" ContentPlaceHolderID="Main" runat="server">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label class="col-sm-2 control-label">
                        学籍状态</label>
                    <div class="col-sm-4">
                        <select id="STU_REGISTER" name="STU_REGISTER" class="form-control" ddl_name='ddl_xjzt'
                            d_value='' title="学籍状态" show_type="t">
                        </select>
                    </div>
                    <label class="col-sm-2 control-label">
                        异动时间</label>
                    <div class="col-sm-4" style="position: relative; z-index: 9999">
                        <input name="STU_DIFFDATE" id="STU_DIFFDATE" type="text" class="form-control datep"
                            placeholder="异动时间" />
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <label class="col-sm-2 control-label">
                        饭卡号</label>
                    <div class="col-sm-4">
                        <input name="STU_RICE_CARD" id="STU_RICE_CARD" type="text" class="form-control" placeholder="饭卡号" />
                        <label id="lab_IS_NONE_STU_RICE_CARD" for="IS_NONE_STU_RICE_CARD">
                            <input type="checkbox" name="IS_NONE_STU_RICE_CARD" id="IS_NONE_STU_RICE_CARD" value="000000"
                                onclick="click_IS_NONE_STU_RICE_CARD(this);" />
                            无
                        </label>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-sm-4">
                        ...
                        <label for="IS_NONE_NEW_BANKCODE">
                            <input type="checkbox" name="IS_NONE_NEW_BANKCODE" id="IS_NONE_NEW_BANKCODE" value="000000"
                                onclick="click_IS_NONE_NEW_BANKCODE(this);" />
                            无
                        </label>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-sm-4">
                        ...
                        <label for="IS_NONE_NEW_IDCARDNO">
                            <input type="checkbox" name="IS_NONE_NEW_IDCARDNO" id="IS_NONE_NEW_IDCARDNO" value="000000"
                                onclick="click_IS_NONE_NEW_IDCARDNO(this);" />
                            无
                        </label>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-sm-2 control-label">
                        辅导员核对备注</label>
                    <div class="col-sm-10">
                        <textarea class="form-control" id="REMARK" name="REMARK" rows="3" placeholder="辅导员核对备注"></textarea>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-sm-2 control-label">
                        院级核对备注</label>
                    <div class="col-sm-10">
                        <textarea class="form-control" id="REMARK_Y" name="REMARK_Y" rows="3" placeholder="院级核对备注"></textarea>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <%--                <button type="button" class="btn btn-primary btn-save" id="btnCheck">
                    如信息有误，点击此按钮</button>--%>
    ...
    <div class="modal fade" id="tableModal_Check">
        <div class="modal-dialog modal-dw60">
            <form action="#" method="post" id="form_check" name="form_check" class="modal-content form-horizontal"
            onsubmit="return false;">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span></button>
                ...
                <div class="form-horizontal box-body">
                    ...
                    <div class="form-group">
                        <label class="col-sm-2 control-label">
                            新身份证号</label>
                        <div class="col-sm-4">
                            <input name="NEW_IDCARDNO_CHECK" id="NEW_IDCARDNO_CHECK" type="text" class="form-control"
                                placeholder="新身份证号" maxlength="18" />
                        </div>
                        <label class="col-sm-2 control-label">
                            确认身份证号</label>
                        <div class="col-sm-4">
                            <input name="NEW_IDCARDNO_CHECK_2" id="NEW_IDCARDNO_CHECK_2" type="text" class="form-control"
                                placeholder="确认身份证号" maxlength="18" />
                        </div>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-primary btn-save" id="btnCheckInfo">
                    确认</button>
                <button type="button" class="btn btn-default pull-left" data-dismiss="modal">
                    关闭</button>
            </div>
            ...
            _content.on('click', "#view", function () {
                ...
                if (data) {
                    if (data.OID) {
                        ...
                        //获得备注信息
                        var remarkdata = AjaxUtils.getResponseText("CheckList.aspx?optype=get_remark&seq_no=" + data.SEQ_NO);
                        if (remarkdata) {
                            var remarkdata_json = eval("(" + remarkdata + ")");
                            $("#REMARK").val(remarkdata_json.REMARK);
                            $("#REMARK_Y").val(remarkdata_json.REMARK_Y);
                        }
                        //获得学生基础信息
                        ...
                        if (studata) {
                            ...
                            $("#STU_REGISTER").val(studata_json.REGISTER);
                            $("#STU_DIFFDATE").val(studata_json.DIFFDATE);
                            $("#STU_RICE_CARD").val(studata_json.RICE_CARD);
                            if (studata_json.RICE_CARD.length > 0) {
                                $("#lab_IS_NONE_STU_RICE_CARD").hide();
                            }
                            else {
                                $("#lab_IS_NONE_STU_RICE_CARD").show();
                            }
                        }
                        //不可编辑
                        ...
                        $("#IS_NONE_STU_RICE_CARD").attr("checked", false);
                        $("#IS_NONE_NEW_BANKCODE").attr("checked", false);
                        $("#IS_NONE_NEW_IDCARDNO").attr("checked", false);
                        _tableModal.modal();
                    }
                }
            });
                ...
                if (data) {
                    if (data.OID) {
                        ...
                        if ("<%=user.User_Role %>" == "F") {
                            ...
                            //20180319：添加 学生学籍 以及 异动时间
                            $("#STU_REGISTER").attr("disabled", false);
                            $("#STU_DIFFDATE").attr("disabled", false);
                        }
                        //ZZ 20180324 修改：新增院级人员的编辑（与辅导员一样）
                        if ("<%=user.User_Role %>" == "Y") {
                            $("#REMARK_Y").attr("disabled", false);
                            //20180319：添加 学生学籍 以及 异动时间
                            $("#STU_REGISTER").attr("disabled", false);
                            $("#STU_DIFFDATE").attr("disabled", false);
                        }
                        //ZZ 20171213 修改：手机号、身份证号、银行卡号 在学生或者辅导员看的时候 可以编辑
                        if ("<%=user.User_Role %>" == "F" || "<%=user.User_Role %>" == "S" || "<%=user.User_Role %>" == "Y") {
                            //学生核对时只能修改饭卡号、银行卡号、身份证号。
                            //$("#STU_PHONE").attr("disabled", false);
                            $("#STU_RICE_CARD").attr("disabled", false);
                            $("#NEW_BANKCODE").attr("disabled", false);
                            $("#NEW_IDCARDNO").attr("disabled", false);
                            $("#IS_NONE_STU_RICE_CARD").attr("disabled", false);
                            $("#IS_NONE_NEW_BANKCODE").attr("disabled", false);
                            $("#IS_NONE_NEW_IDCARDNO").attr("disabled", false);
                        }
                        //显示按钮
                        $("#btnCheck").show();
                        $("#btnSave").show();
    ...
    <script type="text/javascript">
        ...
        function loadModalPageDataInit() {
            ...
            DropDownUtils.initDropDown("STU_REGISTER");
        }
    </script>
    <!-- 编辑页数据初始化事件 结束-->
    <!-- 编辑页验证事件 开始-->
    <script type="text/javascript">
        function loadModalPageValidate() {
            ...
            LimitUtils.onlyNum("STU_RICE_CARD"); //代码限制只能录入数字
            LimitUtils.onlyNum("NEW_BANKCODE"); //代码限制只能录入数字
            LimitUtils.onlyNum("NEW_IDCARDNO"); //代码限制只能录入数字
        }
    </script>
    <!-- 编辑页验证事件 结束-->
    <!-- 自定义实现JS 开始-->
    <script type="text/javascript">
        ...
        function SaveData() {
            ...
            if (NEW_IDCARDNO && NEW_IDCARDNO != "000000") {
                //校验身份证号是否符合规则
                var result = AjaxUtils.getResponseText('/AdminLTE_Mod/Common/ComPage/AjaxHandlePage.aspx?optype=check_idnumber&idno=' + NEW_IDCARDNO);
                if (result.length > 0) {
                    easyAlert.timeShow({
                        "content": "身份证格式不正确！",
                        "duration": 2,
            ...
            var STU_RICE_CARD = $("#STU_RICE_CARD").val();
            if (PROJECT_CLASS == "LIFE") {
                if (STU_RICE_CARD.length == 0) {
                    easyAlert.timeShow({
                        "content": "生活补贴类时，饭卡号不能都为空！",
                        "duration": 2,
                        "type": "info"
                    });
                    return;
            ...
            //辅导员或者学院核对时，核对数据不能为空。
            if ("<%=user.User_Role %>" == "F" || "<%=user.User_Role %>" == "Y") {
                var REMARK = $("#REMARK").val();
                var REMARK_Y = $("#REMARK_Y").val();
                var STU_REGISTER = DropDownUtils.getDropDownValue("STU_REGISTER");
                var STU_DIFFDATE = $("#STU_DIFFDATE").val();
                if (STU_REGISTER.length == 0) {
                    easyAlert.timeShow({
                        "content": "非学生核对时，学籍状态不能为空！",
                        "duration": 2,
                        "type": "info"
                    });
                    return;
                ...
                if (STU_REGISTER != "01") {
                    if (STU_DIFFDATE.length == 0) {
                        easyAlert.timeShow({
                            "content": "非学生核对时，异动时间不能为空！",
                            "duration": 2,
                            "type": "info"
                        });
                        return;
                ...
                if ("<%=user.User_Role %>" == "Y") {
                    if (REMARK_Y.length == 0) {
                        easyAlert.timeShow({
                            "content": "学院核对时，学院备注不能为空！",
                            "duration": 2,
                            "type": "info"
                        });
                        return;
        ...
        function click_IS_NONE_STU_RICE_CARD(obj) {
            if (obj.checked) {
                $("#STU_RICE_CARD").val(obj.value);
                $("#STU_RICE_CARD").attr("disabled", true);
            }
            else {
                $("#STU_RICE_CARD").val("");
                $("#STU_RICE_CARD").attr("disabled", false);
        ...
        function click_IS_NONE_NEW_BANKCODE(obj) {
            if (obj.checked) {
                $("#NEW_BANKCODE").val(obj.value);
                $("#NEW_BANKCODE").attr("disabled", true);
            }
            else {
                $("#NEW_BANKCODE").val("");
                $("#NEW_BANKCODE").attr("disabled", false);
        ...
        function click_IS_NONE_NEW_IDCARDNO(obj) {
            if (obj.checked) {
                $("#NEW_IDCARDNO").val(obj.value);
                $("#NEW_IDCARDNO").attr("disabled", true);
            }
            else {
                $("#NEW_IDCARDNO").val("");
                $("#NEW_IDCARDNO").attr("disabled", false);
