...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script type="text/javascript">
        ...
        window.onload = function () {
            ...
            DropDownUtils.initDropDown("CHANGE_LEVEL");
        ...
        function loadTableList() {
            ...
            mainList = tablePackageMany.createOne({
                ...
                hasBtns: ["reload",
                    ...
                    { type: "userDefined", id: "btn-list-change", title: "修改等级", className: "btn-primary", attr: { "data-action": "", "data-other": "nothing" } }
				], //需要的按钮
                hasCtrl: {
                    "buildModal": false
                }
            ...
            //修改等级
            _content.on('click', "#btn-list-change", function () {
                $("#CHANGE_LEVEL").val("");
                $("#NOTES").val("");
                //辅导员、院级领导操作
                if ('<%=user.User_Role %>' != 'F' && '<%=user.User_Role %>' != 'Y') {
                    easyAlert.timeShow({
                        "content": "对不起，您没有此操作权限！",
                        "duration": 2,
                        "type": "danger"
                    });
                    return;
                ...
                var data = mainList.selectSingle();
                if (data) {
                    var result = AjaxUtils.getResponseText('?optype=checkchg&id=' + data.OID);
                    if (result.length > 0) {
                        easyAlert.timeShow({
                            "content": result,
                            "duration": 2,
    ...
    <script type="text/javascript">
        function ChangeLevel() {
            if (!($("#change_edit").valid())) {
                return;
            }
            var result = AjaxUtils.getResponseText('?optype=change&id=' + $("#OID").val() + '&level=' + $("#CHANGE_LEVEL").val() + '&notes=' + $("#NOTES").val());
            if (result.length > 0) {
                easyAlert.timeShow({
                    "content": result,
                    "duration": 2,
            ...
            //$.post(OptimizeUtils.FormatUrl("?optype=change&id=" + $("#OID").val()), $("#change_edit").serialize(), function (msg) {
            //    if (msg.length > 0) {
            //        easyAlert.timeShow({
            //            "content": msg,
            //            "duration": 2,
            ...
            <form method="post" id="change_edit" name="change_edit" onsubmit='return false;' class="modal-content form-horizontal">
                ...
                <div class="modal-body">
                    <div class="form-group" id="step1">
                        <label class="col-sm-2 control-label">推荐档次<span style="color: Red;">*</span></label>
                        <div class="col-sm-10">
                            <select id="CHANGE_LEVEL" name="CHANGE_LEVEL" class="form-control" ddl_name='ddl_dst_level' d_value='' title="推荐档次" show_type='t' required></select>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-sm-2 control-label">修改原因<span style="color: Red;">*</span></label>
                        <div class="col-sm-10">
                            <textarea id="NOTES" name="NOTES" cols="4" rows="4" class="form-control" placeholder="修改原因" required></textarea>
                        </div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary btn-save" id="btnChange">
                        提交</button>
